 ...
 * return
 *
 * this function is for jamie's multi channel dma through single channel 
 *
 */
...
static void axienet_dma_bd_release(struct net_device *ndev)
{
	...
	//jamie: DMA Holder do the rest, and else do nothing.
...
//jamie patch: support packet/byte send counter
...
//jamie patch: here should tell system, there are more space
...
//jamie patch: use main dma channel to transfer
	int dma_channel_number;
	of_property_read_u32( lp->dev->of_node, "meshsr,dma_channel", &dma_channel_number);
	if(dma_channel_number != 0)
	{
	...
	//jamie patch add channel support
...
//jamie patch
	...
	//dev_info(&pdev->dev, "DMA Multi Channel Number =0x%x\n", dma_channel_number);
	if(dma_channel_number == 0)
	{
